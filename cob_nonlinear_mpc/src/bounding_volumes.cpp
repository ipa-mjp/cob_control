...
std::map<string,vector<vector<SX>>> BoundingVolume::generate_bounding_volumes(Robot &robot, ForwardKinematics &fk)
{
    ...
    // Get bounding volume forward kinematics
        for(int i=0; i<transform_vec_bvh_.size(); i++)
        {
            ...
            if(i-1<0)
            {
                SX transform = mtimes(fk.fk_vector_.at(i),bvh.T);
                ...
                if(bvh.constraint)
                {
                    bvh_arm.clear();
                    tmp.clear();
                    transform = mtimes(fk.fk_vector_.at(i),bvh.BVH_p);
            ...
            {
                bvh_arm.clear();
                SX transform = mtimes(fk.fk_vector_.at(i-1),bvh.T);
                SX tmp = SX::vertcat({transform(0,3), transform(1,3), transform(2,3)});
                bvh_arm.push_back(tmp);
                bvh_matrix[bvh.link].push_back(bvh_arm);
                bvh_arm.clear();
                ...
                {
                    tmp.clear();
                    transform = mtimes(fk.fk_vector_.at(i-1),bvh.BVH_p);
                    tmp = SX::vertcat({transform(0,3), transform(1,3), transform(2,3)});
                    bvh_arm.push_back(tmp);
                    bvh_matrix[bvh.link].push_back(bvh_arm);
                }
        ...
        if(robot.base_active_)
        {
            for(int i=0; i<bvb_positions_.size(); i++)
            {
                ...
                SX tmp = SX::vertcat({fk.fk_base_(0,3), fk.fk_base_(1,3), fk.fk_base_(2,3)+bvb_positions_.at(i)});

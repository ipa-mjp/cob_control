...
void ForwardKinematics::symbolic_fk(Robot &robot, SX u, SX x)
{
    // Casadi symbolics
    ...
    ////generic rotation matrix around z and translation vector for x and y
        T(0,0) = cos(x(2)); T(0,1) = -sin(x(2));  T(0,2) = 0.0; T(0,3) = x(0);
        T(1,0) = sin(x(2)); T(1,1) = cos(x(2));   T(1,2) = 0.0; T(1,3) = x(1);
    ...
    for(int i=0;i<robot.joint_frames.size();i++)
    {
        ...
        //still needs to be improved... if the chain is composed by joint than not joint than joint again this is going to be wrong
            T(0,0) = rot(0,0)*cos(x(i+3))+rot(0,1)*sin(x(i+3));
            T(0,1) = -rot(0,0)*sin(x(i+3))+rot(0,1)*cos(x(i+3));
            T(0,2) = rot(0,2); T(0,3) = pos.x();
            T(1,0) = rot(1,0)*cos(x(i+3))+rot(1,1)*sin(x(i+3));
            T(1,1) = -rot(1,0)*sin(x(i+3))+rot(1,1)*cos(x(i+3));
            T(1,2) = rot(1,2); T(1,3) = pos.y();
            T(2,0) = rot(2,0)*cos(x(i+3))+rot(2,1)*sin(x(i+3));
            T(2,1) = -rot(2,0)*sin(x(i+3))+rot(2,1)*cos(x(i+3));
            T(2,2) = rot(2,2); T(2,3) = pos.z();
            T(3,0) = 0.0; T(3,1) = 0.0; T(3,2) = 0.0; T(3,3) = 1.0;
        }
        else{
            T(0,0) = rot(0,0)*cos(x(i))+rot(0,1)*sin(x(i));
            T(0,1) = -rot(0,0)*sin(x(i))+rot(0,1)*cos(x(i));
            T(0,2) = rot(0,2); T(0,3) = pos.x();
            T(1,0) = rot(1,0)*cos(x(i))+rot(1,1)*sin(x(i));
            T(1,1) = -rot(1,0)*sin(x(i))+rot(1,1)*cos(x(i));
            T(1,2) = rot(1,2); T(1,3) = pos.y();
            T(2,0) = rot(2,0)*cos(x(i))+rot(2,1)*sin(x(i));
            T(2,1) = -rot(2,0)*sin(x(i))+rot(2,1)*cos(x(i));
            T(2,2) = rot(2,2); T(2,3) = pos.z();
            T(3,0) = 0.0; T(3,1) = 0.0; T(3,2) = 0.0; T(3,3) = 1.0;
        }

        T_BVH fk_transform;
        fk_transform.link = robot.kinematic_chain.getSegment(i).getName();
        fk_transform.T = T;
    ...
    for(int i=0; i < transformation_vector_.size(); i++)
    {
        ...
        if(robot.base_active_)
        {
            if(i==0)
            {
                ...
                tmp = mtimes(fk_base_,transformation_vector_.at(i).T);
            }
            else
            {
